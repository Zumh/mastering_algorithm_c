// Description: Illustrate using a binary tree (see Chapter 9)
...
static void print_preorder(const BiTreeNode *node){
    // Display the binary tree rooted at the spcified node in preorder.
...
static void print_inorder(const BiTreeNode *node){
    ...
    // Display the binary tree rooted at the spcified node in inorder.
    if (!bitree_is_eob(node)){
...
int main(){
    ...
    fprintf(stdout, "Destroying the binary tree\n");
    bitree_destroy(&tree);
