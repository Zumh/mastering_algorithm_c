...
int cover(Set *members, Set *subsets, Set *covering){
	...
	while(set_size(members) > 0 && set_size(subsets) > 0){
		...
		// A covering is a not possible if there was no intersection.
		if(max_size == 0){
			return 1;
		}
		...
		// Remove the subste from the set of candidate subsets.
		if (set_remove(subsets, (void **)&subset) != 0){
			return -1;
		}

	}
	// No covering its possible if there are still noncovered members.
